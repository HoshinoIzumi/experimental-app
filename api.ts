...
export const getAllTodos = async (): Promise<ITask[]> => {
  const res = await fetch(`${baseUrl}/tasks?_sort=createdAt&_order=desc`, { cache: 'no-store' })
  if (!res.ok) throw new Error(`GET /tasks failed: ${res.status}`)
  return res.json()
}

export const addNewTodo = async (title: string, description?: string): Promise<ITask> => {
...
export const updateTodo = async (id: string, updates: Partial<ITask>): Promise<ITask> => {
